...
const register = async (req, res, next) => {
    ...
    // Validate role - only allow consumer or creator, default to consumer
    const validRoles = ['consumer', 'creator'];
    const userRole = validRoles.includes(role) ? role : 'consumer';
    try {
        const hashedPassword = await bcrypt.hash(password, 10);
        const [result] = await pool.query('INSERT INTO users (email, password, name, role) VALUES (?, ?, ?, ?)', [email, hashedPassword, name, userRole]);
