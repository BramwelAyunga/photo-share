...
const getPublicMedia = async ({ page = 1, limit = 10, filter, search }) => {
    let query = `
        SELECT m.id, m.creator_id, m.blob_url, m.created_at, m.title, m.caption, 
               m.location, m.people, m.thumbnail_blob_url, u.name as creator_name, 
    ...
    if (search) {
        query += ` AND (m.title LIKE ? OR m.caption LIKE ? OR u.name LIKE ? OR m.location LIKE ?)`;
